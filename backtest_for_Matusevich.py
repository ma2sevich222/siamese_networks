...
result_filename = f'{DESTINATION_ROOT}/{out_root}/sel_dist_{source_filename[:-4]}_step{step}'
...
for sell_after in trange(int(1/step), int(round(df.Distance.max(), 1)/step)):
    for buy_before in range(int(round(df.Distance.min(), 1)/step), int(1/step)):
        ...
        df.to_csv(f'{DESTINATION_ROOT}/{out_root}/signals_{FILENAME[:-4]}_patern{PATTERN_SIZE}_extrw{EXTR_WINDOW}_overlap{OVERLAP}_step{step}_{buy_before * step}_{sell_after * step}.csv')
        ...
        stats.to_csv(f'{DESTINATION_ROOT}/{out_root}/stats_{FILENAME[:-4]}_patern{PATTERN_SIZE}_extrw{EXTR_WINDOW}_overlap{OVERLAP}_step{step}_{buy_before * step}_{sell_after * step}.txt')

        df_stats = df_stats.append(stats, ignore_index=True)
        df_stats.loc[i, 'Net Profit [$]'] = df_stats.loc[i, 'Equity Final [$]'] - deposit - df_stats.loc[i, '# Trades'] * comm
