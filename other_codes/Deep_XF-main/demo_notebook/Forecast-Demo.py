...
ExploratoryDataAnalysis.plot_dataset(
    df=model_df,
    title="PJM East (PJME) Region: estimated energy consumption in Megawatts (MW)",
)
...
df_full_features = Features.generate_date_time_features_hour(
    model_df, ["hour", "month", "day", "day_of_week", "week_of_year"]
)
...
df_full_features = Features.generate_cyclic_features(
    df_full_features, "day_of_week", 7, 0
)
df_full_features = Features.generate_cyclic_features(df_full_features, "month", 12, 1)
df_full_features = Features.generate_cyclic_features(
    df_full_features, "week_of_year", 52, 0
)
...
(
    ...
    learning_rate,
    weight_decay,
) = Forecast.hyperparameter_config(
    hidden_dim=64,
    layer_dim=3,
    batch_size=64,
    dropout=0.2,
...
forecasted_data, ff_full_features, ff_full_features_ = Forecast.forecast(
    model_df, ts, fc, opt, scaler, period=25, fq="h", select_scaler=select_scaler,
)
...
Helper.explainable_forecast(
    df_full_features,
    ff_full_features_,
    fc,
    specific_prediction_sample_to_explain=df_full_features.shape[0] + 2,
    input_label_index_value=0,
    num_labels=1,
)
