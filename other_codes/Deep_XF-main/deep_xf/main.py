...
class Forecast:
    ...
    forecast_window = (
        globals()
    )  # no. of timesteps/points to be used for the forecasting model and nowcasting period
    ...
    def forecast(
        df, ts, fc, opt, scaler, period: int, fq: str, select_scaler=select_scaler,
    ):
        ...
        if (
            frequency in ["h", "H", "s", "S", "min", "MIN", "n", "N"]
            ...
            ff_full_features = Features.generate_date_time_features_hour(
                ff_df, ["hour", "month", "day", "day_of_week", "week_of_year"]
            )
            ff_full_features = Features.generate_cyclic_features(
                ff_full_features, "hour", 24, 0
            )
            ff_full_features = Features.generate_cyclic_features(
                ff_full_features, "day_of_week", 7, 0
            )
            ff_full_features = Features.generate_cyclic_features(
                ff_full_features, "month", 12, 1
            )
            ff_full_features = Features.generate_cyclic_features(
                ff_full_features, "week_of_year", 52, 0
            )

            ff_full_features = Features.generate_other_related_features(
                df=ff_full_features
            )
            ...
            ff_full_features = Features.generate_date_time_features_month(
                ff_df, ["month", "day_of_week", "week_of_year"]
            )
            ff_full_features = Features.generate_cyclic_features(
                ff_full_features, "day_of_week", 7, 0
            )
        ...
        predictions = opt.predict(unseen_loader, batch_size=1, n_features=input_dim)
    ...
    def train(
        ...
        (
            ...
            X_train, X_val, X_test, y_train, y_val, y_test, select_scaler
        )
        ...
        opt = Optimization(
            device=device, model=model, loss_fn=loss_fn, optimizer=optimizer
        )
        opt.train(
            train_loader,
            val_loader,
        ...
        df_baseline = Helper.build_baseline_model(
            df, split_ratio, target_col
        )  # df_feature, 0.2, 'value'
