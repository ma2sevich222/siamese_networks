...
class Forecasting_Models:
    ...
    class BiRNNModel(nn.Module):
        ...
        def forward(self, x):
            ...
            ).requires_grad_()  # .to(device) # 2 for bidirection
            c0 = torch.zeros(
                self.layer_dim * 2, x.size(0), self.hidden_dim
    ...
    class BiGRUModel(nn.Module):
        ...
        def forward(self, x):
            ...
            h0 = torch.zeros(
                self.layer_dim * 2, x.size(0), self.hidden_dim
            ).requires_grad_()  # 2 for bidirection
