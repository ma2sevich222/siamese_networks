...
class Helper:
    ...
    def train_val_test_split(df, target_col, test_ratio):
        ...
        print(
            f"Predictors: Train-{(X_train.shape)}, Val-{(X_val.shape)}, Test-{(X_test.shape)}"
        )
        print(
            f"Response: Train-{(y_train.shape)}, Val-{(y_val.shape)}, Test-{(y_test.shape)}"
        )
    ...
    def format_predictions(predictions, values, df_test, scaler):
        ...
        df_result = pd.DataFrame(
            data={"value": vals, "prediction": preds},
            index=df_test.head(len(vals)).index,
        )
        df_result = df_result.sort_index()
        df_result = Helper.inverse_transform(
    ...
    def calculate_metrics(df):
        metrics_one = {
            ...
            "fc_bias": (
                np.sum(np.array(df.value) - np.array(df.prediction))
                * 1.0
                / len(df.value)
            ),
        ...
        print(
            "Symmetric Mean Absolute Percentage Error (sMAPE):   ", metrics_two["smape"]
        )
    ...
    def plot_metrics(metrics_one, metrics_two):
        ...
        axes.set(
            xlabel="Metrics",
            ylabel="Value",
            title="Model Peformance wrt. their corresponding metrics",
        )
    ...
    def make_future_df(df, ts, period: int, fq: str):
        ...
        if str(fq) == "h" or str(fq) == "H" or interval is not None:  # hourly
            ...
            last_date = df.index.max()
            dates = pd.date_range(
                start=last_date, periods=period + 1, freq=fq
            )  # Add 1 incase last date is included
            dates = dates[dates > last_date]  # Drop start if equals last_date
            dates = dates[:period]  # Return correct number of periods
            ...
            str(fq) == "y" or str(fq) == "Y" or str(fq) == "A" or interval is not None
        ):  # yearly
            last_date = df.index.max()
            dates = pd.date_range(
                start=last_date, periods=period + 1, freq=fq
    ...
    def compare_ml_models_and_plot():
        ...
        # Metrics
        print(
            f"R2_nd_RMSE LR MODEL: {CompareModels.R2AndRMSE(y_test=y_test, y_pred=y_pred)}"
        )
        ...
        print(
            f"R2_nd_RMSE RF MODEL: {CompareModels.R2AndRMSE(y_test=y_test, y_pred=y_pred)}"
        )
        ...
        print(
            f"R2_nd_RMSE XGB MODEL: {CompareModels.R2AndRMSE(y_test=y_test, y_pred=y_pred)}"
        )
    ...
    def explainable_forecast(
        df,
        ff,
        fc,
        specific_prediction_sample_to_explain: int,
        ...
        XpredictInputData = X.iloc[
            specific_prediction_sample_to_explain, :
        ]  # X[specific_prediction_sample_to_explain,:]
        ...
        explainer = shap.KernelExplainer(
            model=model_nn.predict, data=X.head(50), link="identity"
        )  # data = X[0:50]
        ...
        shap_value_single = explainer.shap_values(
            X=X.iloc[specific_prediction_sample_to_explain, :], nsamples=100
        )  # X[specific_prediction_sample_to_explain,:]
...
class Optimization:
    ...
    def evaluate(
        self, test_loader, batch_size=1, n_features=1,
    ):
    ...
    def predict(
        self, forecast_loader, batch_size=1, n_features=1,
    ):
    ...
    def forecast_torch(
        self, full_loader, n_steps: int, batch_size=1, n_features=1,
    ):
