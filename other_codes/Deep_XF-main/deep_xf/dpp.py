...
class Preprocessing:
    ...
    def datetime_merge(df, datecol: str, timecol: str):
        ...
        if df[datecol].dtypes == "O" or df[timecol].dtypes == "O":
            ...
            df["TS"] = df.apply(
                lambda x: pd.datetime.combine(x["datecol"], x["timecol"]), 1
            )
            # move newly create TS column back to start postion
            cols = list(df)
            cols.pop(cols.index("TS"))
...
class DescriptiveStatistics:
    ...
    def desc_all(df, start_sensor_column_index):
        ...
        return df.iloc[
            :, start_sensor_column_index : len(df.columns)
...
class StatisticalEvaluation:
    ...
    def normality_test(df, start_sensor_column_index):
        ...
        print(
            "Shapiro-Wilk Normality test (W test statistic, the p-value) =======> {}  ".format(
                shapiro(df.iloc[:, start_sensor_column_index : len(df.columns)])
            )
        )
        print(
            "Jarque-Bera Normality test (W test statistic, the p-value) =======> {}".format(
                jarque_bera(df.iloc[:, start_sensor_column_index : len(df.columns)])
            )
        )
...
class Features:
    ...
    def generate_date_time_features_ohe(df, cols: list()):
        ...
        def onehot_encode_pd(df_features=df_features, cols=cols):
            ...
            df_with_features_dummies = pd.concat(
                [df_features, df_with_dummies.iloc[:, 1:]], axis=1
            )
