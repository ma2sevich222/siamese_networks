...
class Denoise:
    ...
    def plot_and_write(
        ...
        filter_ts_signal = Denoise.overlay_filter(
            df.iloc[:, featurecol_index:],
            cutoff_high,
            cutoff_low,
            nyq,
            powerline,
        ...
        filter_ts_signal_dataset = pd.DataFrame(
            np.array(filter_ts_signal), columns=cols
        )
        filtered_dataset = pd.concat(
            [
                df.iloc[:, :featurecol_index].reset_index(),
                filter_ts_signal_dataset.reset_index(),
            ],
            axis=1,
        )
