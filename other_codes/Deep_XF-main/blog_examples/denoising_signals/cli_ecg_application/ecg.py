...
parser.add_argument(
    "-d", "--data", type=str, required=True, help="Path to raw ECG signal CSV data"
)
parser.add_argument(
    "-o",
    "--output-path",
...
parser.add_argument(
    ...
    default=500,
    dest="fs",
    help="Sampling frequency of raw ECG signal",
)
parser.add_argument(
    "-s",
    "--snr-algorithm",
...
if __name__ == "__main__":
    ...
    if args.output is not None:
        filtered_df = pd.DataFrame(
            data=filtered, index=signal.index, columns=signal.columns
        )
    ...
    if args.snr_alg == "default":
        ...
        # Calculate SNR
        snr = np.where(
            compressed_data.std(axis=0, ddof=0) == 0, 0, compressed_data.mean(axis=0)
        )
