...
Train_df, Eval_df, train_dates, test_dates = data_load_OHLCV(SOURCE_ROOT, FILENAME, START_TEST, END_TEST, PATTERN_SIZE,
                                                             TRAIN_WINDOW)  # загрузка данных
...
with torch.no_grad():
    for indexI, eval_arr in enumerate(tqdm(eval_normlzd)):
        ...
        output1, output2, output3 = net(anchor.cuda().permute(0, 3, 1, 2), eval_arr_r.cuda().permute(0, 3, 1, 2),
