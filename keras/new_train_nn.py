...
early_stopping = EarlyStopping(monitor='loss', patience=100, verbose=0, restore_best_weights=True, mode='min')
# reduce_lr = ReduceLROnPlateau(monitor='accuracy', factor=0.9, patience=12, min_lr=1e-07, verbose=1, mode='max')
checkpoint = ModelCheckpoint(f'{MODELS_ROOT}/{TICKER}_model.h5', monitor='accuracy', verbose=0, save_best_only=True)
...
Predictions.to_csv(f'{DESTINATION_ROOT}/test_results_extr_window{EXTR_WINDOW}'
                   f'_latent_dim{latent_dim}'
                   f'_pattern_size{PATTERN_SIZE}.csv', index=True)
