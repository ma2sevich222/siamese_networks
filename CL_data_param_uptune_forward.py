...
out_data_root = f"{source_file_name[:-4]}_CL_data_optune_{date_xprmnt}_epoch_{n_trials}"
...
def objective(trial):
    ...
    extr_window = trial.suggest_int("extr_window", 10, 60)
    pattern_size = trial.suggest_int("pattern_size", 5, 50)
    overlap = trial.suggest_int("overlap", 0, 5, step=3)
    train_window = trial.suggest_int("train_window", 2000, 14000, step=1000)
    select_dist_window = trial.suggest_int("select_dist_window", 2000, 14000, step=1000)
    forward_window = trial.suggest_categorical("forward_window", ["2000", "14000"])
